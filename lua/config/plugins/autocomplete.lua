...
local emmet_filetypes = make_set("html", "eruby", "vue", "templ", "heex", "markdown", "htmldjango", "jinja")
...
local function cmp_opts()
	...
	local mappings = {
		...
		["<C-Space>"] = m.complete({}),
		["<Tab>"] = m(on_tab, { "i", "s" }),
		["<C-k>"] = m.confirm({ select = true, behavior = cmp.ConfirmBehavior.Replace }),
		["<C-a>"] = m.confirm({ select = true, behavior = cmp.ConfirmBehavior.Replace }),
		["<S-Tab>"] = m(on_s_tab, { "i", "s" }),
	}
